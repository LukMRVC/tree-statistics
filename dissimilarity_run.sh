...
# ./resources/ted-ds-dist $P trees.bracket 90
...
for it in {1..3}; do
    ./target/release/tree-statistics --quiet -d $P/trees.bracket lower-bound -q $P/1query.csv --output $QUERY_RESULT | tail -n 18 > $QUERY_RESULT/query_times_${it}.txt
done
./resources/query_validate_3 $P/trees.bracket $P/1query.csv $QUERY_RESULT/Lblint_candidates.csv $QUERY_RESULT/Sed_candidates.csv $QUERY_RESULT/Structural_candidates.csv > $QUERY_RESULT/verified-all.txt 2> /dev/null
