...
def pick_queries_gen(dataset):
    ...
    with open(join(BASEPATH, dataset, "trees.bracket")) as f:
        ...
        df = pl.read_csv(
            join(BASEPATH, dataset, f"distances-tjoin.txt"),
            has_header=False,
        ...
        with open(join(BASEPATH, dataset, 'queries.csv'), "w") as f:
            for tid, tau in qs.items():
                if tree_sizes[tid] < tau:
                    print(
                        f"Tree {tid}: size {tree_sizes[tid]} < tau {tau}."
                    )
...
@click.command()
# add nargs arugments for datasets
@click.argument("datasets", nargs=-1)
def cli(datasets):
    """Generate queries for the given dataset."""
    for dataset in datasets:
        pick_queries_gen(dataset)
    


if __name__ == "__main__":
    # Run the script with click
    cli()