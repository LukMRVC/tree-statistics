...
for dataset, (min_size, max_size) in datasets_most_frequest_sizes.items():
    ...
    if single_percent_len <= 10:
        ...
        min_results = single_percent_len - 5
        max_results = single_percent_len + 5
    else:
        min_results = single_percent_len - 10
        max_results = single_percent_len + 10
    ...
    df = pl.read_csv(join(BASEPATH, f"distances-{dataset}.csv"), has_header=False, schema={"T1": pl.Int32(), "T2": pl.Int32(), "K": pl.Int32()},)
    ...
    for tau in range(1, mx + 1):
        g = df.filter( (df['K'] < tau) & (df['T1'].is_in(pickable_tids)) ).group_by("T1").agg(pl.len().alias("cnt"))
        g = g.filter((g["cnt"] >= min_results) & (g["cnt"] < max_results))
        # g = g.filter((g["cnt"] > 1) & (g["cnt"] < max_results))

        print('Tau = ', tau,  g.head(5))
        ...
        g = df.filter( (df['K'] < tau) & (df['T2'].is_in(pickable_tids)) ).group_by("T2").agg(pl.len().alias("cnt"))
        g = g.filter((g["cnt"] >= min_results) & (g["cnt"] < max_results))
